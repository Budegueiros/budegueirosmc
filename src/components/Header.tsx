...
function Header() {
  ...
  const navLinks = [
    ...
    { path: '/dashboard', label: 'Área do Integrante' },
  ];
      ...
      <nav className="container mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8">
          {navLinks.map(({ path, label }) => (
            <Link key={path} to={path} className={`${isActive(path) ? 'text-brand-red' : 'text-white'} hover:text-brand-red transition font-oswald uppercase text-sm tracking-wide`}>{label}</Link>
          ))}
          {/* Sociais */}
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-black/95 border-t border-brand-red/20 px-4 py-6 relative z-50">
          <div className="flex flex-col gap-4 items-center">
            {navLinks.map(({ path, label }) => (
              <Link key={path} to={path} className={`${isActive(path) ? 'text-brand-red' : 'text-white'} hover:text-brand-red transition font-oswald uppercase text-center`} onClick={() => setIsMenuOpen(false)}>{label}</Link>
            ))}
